    ...
    public boolean onCommand(@NotNull CommandSender sender, @NotNull Command cmd, @NotNull String commandLabel, String[] args) {
        ...
        if (cmd.getName().equalsIgnoreCase("gunban") && player.hasPermission("geyseradmintools.gunban")) {
            ...
            if (!offPlayer.hasPlayedBefore()) {
                for (OfflinePlayer bPlayer : Bukkit.getOfflinePlayers()) {
                    if (args[0].equalsIgnoreCase(bPlayer.getName())) {
                        BanData.deleteBan(bPlayer.getUniqueId());
                            player.sendMessage(ChatColor.DARK_AQUA + Messages.get("unban.command.player.message1",bPlayer.getName()));
                    }
                }
            }
        }
